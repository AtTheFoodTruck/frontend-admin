...
const ReceiveOrderList = ({
  ...
  return (
    ...
    <ListGroup.Item className="d-inline-flex align-items-center">
      ...
      <Col className="d-flex justify-content-center">{orderId}</Col>
      <Col className="d-flex justify-content-center">{orderDate}</Col>
      <Col className="d-flex justify-content-center">{orderState}</Col>
      <Col className="d-flex justify-content-center">{userName}</Col>
      <Col id="menu_name">{newArray.join()}</Col>
      <Col>
        {acceptType ? (
          <Button disabled={true} onClick={() => acceptOrder}>
            주문수락
          </Button>
        ) : (
          <Button disabled={false} onClick={() => acceptOrder(orderId)}>
            주문수락
          </Button>
        )}
      </Col>
      <Col>
        {rejectType ? (
          <Button disabled={true} onClick={() => rejectOrder}>
            주문거절
          </Button>
        ) : (
          <Button disabled={false} onClick={() => rejectOrder(orderId)}>
            주문거절
          </Button>
        )}
      </Col>
      <Col>
        {completeType ? (
          <Button disabled={true} onClick={() => completeOrder}>
            조리완료
          </Button>
        ) : (
          <Button disabled={false} onClick={() => completeOrder(orderId)}>
            조리완료
          </Button>
        )}
      </Col>
